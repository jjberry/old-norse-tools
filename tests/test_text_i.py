...
class TestAnnotationFallback:
    """Forms not generated by paradigms should resolve via the fallback table."""
    ...
    def test_fallback_returns_result(self, conn, surface, expected_pos, expected_gloss_fragment):
        results = parse_form(surface, conn)
        assert results, f"No results for '{surface}' (expected annotation fallback)"
        assert all(r["source"] == "annotation" for r in results), (
            f"'{surface}' should come from annotation fallback"
        )
